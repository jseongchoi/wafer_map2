...
이 문서는 FBM observable feature 기반 유사 wafer 검색이 실제 현업 눈에도 의미 있는지 확인하기 위한 최소 리뷰 절차다.

핵심은 모델을 더 복잡하게 만드는 것이 아니라, query wafer와 top-k neighbor를 전문가가 보고 다음 판단을 남기는 것이다.

```text
real/synthetic nearest-neighbor CSV
-> expert review template
-> reviewer decision
-> aggregate metrics
-> feature tuning / morphology / segmentation 우선순위 결정
```

## 현재 위치

이 프로토콜은 Real-Unlabeled Workflow 다음 단계다.

- `scripts/extract_real_unlabeled_features.py`가 observable feature와 nearest-neighbor CSV를 만든다.
- `scripts/extract_real_unlabeled_features.py`는 reference feature가 주어지면 reviewer 입력용 CSV도 함께 만든다.
- `scripts/make_expert_review_template.py`는 이미 만든 nearest-neighbor CSV를 별도로 template CSV/HTML로 바꿀 때 사용한다.
...
- `query_sample_id`: pseudonymized query id
- `rank`: nearest-neighbor rank
- `neighbor_sample_id`: pseudonymized neighbor id
- `distance`: observable feature space distance
...
Real-unlabeled workflow에서 만든 neighbor CSV를 template으로 바꾼다.

Real-unlabeled 실행에서 바로 template을 만들 수 있다.
...
- `next_action_queue`: defect family x failure mode x action 단위 backlog

해석 기준 초안:

- accepted rate가 높고 missed-major가 낮으면 현재 feature retrieval이 triage에 유효하다.
...
- `next_action_queue`에서 `stby_hidden_origin_mismatch`, `shot_phase_layout_mismatch`, `ring_radius_width_mismatch` 같은 항목이 쌓이면 해당 feature family 보강 backlog로 바로 연결한다.

## 보안 규칙

- 실제 wafer raw image/array는 repo에 저장하지 않는다.
- 실제 파일 경로, lot id, wafer id, tool id, recipe, chamber 정보는 `safe_comment`에 쓰지 않는다.
- sample id는 pseudonymized id만 사용한다.
...
1. accepted rate가 충분하면 current feature baseline을 real triage MVP로 유지한다.
2. 특정 family만 약하면 해당 family의 observable feature를 보강한다.
