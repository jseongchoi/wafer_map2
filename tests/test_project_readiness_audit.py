...
def test_project_readiness_audit_scans_shareable_outputs_for_private_paths(tmp_path, monkeypatch):
    module = _load_script()
    monkeypatch.setattr(module, "ROOT", tmp_path)
    report_dir = tmp_path / "outputs" / "reports"
    report_dir.mkdir(parents=True)
    (report_dir / "leaky_manifest.json").write_text(
        json.dumps({"samples": [{"png_path": "C:\\secure\\wafer.png", "allow_workspace_input": True}]}),
        encoding="utf-8",
    )

    findings = module.scan_shareable_outputs(("outputs/reports",))

    assert any("leaky_manifest.json" in finding for finding in findings)
...
def test_project_readiness_audit_marks_real_batch_without_production_metadata_as_check(tmp_path, monkeypatch):
    module = _load_script()
    monkeypatch.setattr(module, "ROOT", tmp_path)
    ...
    report_dir.mkdir(parents=True)
    (private / "real_png_batch_manifest.json").write_text(
        json.dumps(
            {
    ...
    (report_dir / "features.csv").write_text("sample_id,total_fail_density\nproduct_aaaaaaaaaa_wbbbbbbbbbb,0\n", encoding="utf-8")
    (report_dir / "sanity.json").write_text(json.dumps({"samples": [{"sample_id": "product_aaaaaaaaaa_wbbbbbbbbbb", "errors": []}]}), encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "manifest_location": "outputs/private",
    ...
    assert stage8["status"] == "CHECK"
    assert any("--production-run" in note for note in stage8["notes"])
...
def test_project_readiness_audit_checks_real_batch_positive_actual_net_die_count(tmp_path, monkeypatch):
    module = _load_script()
    monkeypatch.setattr(module, "ROOT", tmp_path)
    report_dir = tmp_path / "outputs" / "reports" / "real_png_batch"
    report_dir.mkdir(parents=True)
        ...
        json.dumps(
            {
                ...
                "product_count": 1,
                "explicit_geometry_product_count": 1,
                "actual_net_die_product_count": 0,
                "png_sample_count": 1,
                "geometry_contract": "explicit",
                "manifest_location": "outputs/private",
                "reference_features": True,
    ...
    assert any("positive actual_net_die count mismatch" in note for note in notes)
